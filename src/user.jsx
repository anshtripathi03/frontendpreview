...
function NavItem({ label, id, activeSection, setActiveSection }) {
  ...
  return (
    <button
      ...
      className={`w-full text-left px-3 py-2 rounded ${active ? 'bg-primary bg-gray-50 dark:hover:bg-gray-700' : ' '}`}
    >
      {label}
    </button>
  );
...
export default function EcommerceUserDashboard() {
  ...
  // const handleAddAddress = (addr) => setAddresses(prev => [...prev, { id: `A${prev.length + 1}`, ...addr }]);
    ...
    <div className="min-h-screen dark:bg-gray-900 text-gray-800 dark:text-gray-100 overflow-x-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 md:py-5">
            ...
            {["overview", "orders", "wishlist", "addresses", "account", "settings"].map((section) => (
              <li key={section}>
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded ${activeSection === section ? "bg-gray-300 font-bold" : "hover:bg-gray-200"
                    }`}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1 bg-white dark:bg-gray-800 rounded-lg p-4 shadow hidden md:block">
